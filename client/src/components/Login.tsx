...
export const Login: React.FC<Props> = ({ onLogin }) => {
  ...
  return (
   <div className="min-h-screen bg-[var(--color-bg)] flex flex-col items-center justify-center p-6 relative">
      {/* Theme Toggle */}
      <motion.button
        onClick={toggleTheme}
        whileHover={{ scale: 1.1 }}
        whileTap={{ scale: 0.9 }}
        className="absolute top-6 right-6 w-9 h-9 flex items-center justify-center bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-secondary)] rounded-[10px] shadow-[var(--shadow-card)] transition-all duration-150 cursor-pointer"
